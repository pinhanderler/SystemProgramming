...
printLabel macro printedText, xPos:=<LabelXPos>, yPos:=<LabelYPos>
  ...
  COMMENT *
    ...
    Ends on the symbol, that goes after word COMMENT.
  *
...
.data
  ...
  ErrorMessageOne db "The input is wrong.", 0
  ErrorMessageTwo db "Try again.", 0
  ErrorMessageThree db "I believe in you)", 0

  ; Student related data
  nameText db "Rekechynsky Dmytro", 0
  nameTextXPos equ 40
  nameTextYPos equ 90

  numberText db "9882", 0
  numberTextXPos equ 40
  numberTextYPos equ 120

  dateText db "11.10.1999", 0
  dateTextXPos equ 40
  dateTextYPos equ 150

  ; The password is "abcdefgh"
  password db "dgfa`cbm"
  key db 00000101b
  len dw 8
...
WndProc proc hWnd :dword, uMsg :dword, wParam :dword, lParam :dword
  ...
  .elseif uMsg==WM_COMMAND
    ...
    invoke SendMessage, hEditText, WM_GETTEXT,
      KeyboardHookWParam, addr input
    ; If the length of input and password aren't equal,
    ; there is an error
    cmp ax, len
    jne inputError
    ; Otherwise, compare input with password
    cipherText input
    compareText input, password
    ; If everything is correct -- zero (Z) flag is set to 0
    ; Otherwise (are not equal) -- Z flag is is set to 1
    je inputSuccess
    inputError:
      printLabel ErrorMessageOne, nameTextXPos, nameTextYPos
      printLabel ErrorMessageTwo, numberTextXPos, numberTextYPos
      printLabel ErrorMessageThree, dateTextXPos, dateTextYPos
      jmp stop
    ; Success dialog
    inputSuccess:
      printLabel nameText, nameTextXPos, nameTextYPos
      printLabel numberText, numberTextXPos, numberTextYPos
      printLabel dateText, dateTextXPos, dateTextYPos
